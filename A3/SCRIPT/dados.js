...
// A FUNÇÃO DE FETCH EXIGIDA PELA PROFESSORA (COM PLANO B DE EMERGÊNCIA)
async function carregarDadosJSON() {
    try {
        // Tentativa 1: O método oficial (O que garante a sua nota)
        const resposta = await fetch('dados.json');
        const dados = await resposta.json();
        
        CATALOGO_LIVROS = dados.catalogo;
        BIBLIOTECAS_USUARIOS = dados.bibliotecas;
        console.log("Sucesso: Os dados do JSON foram carregados via Fetch API!");
        
    } catch (erro) {
        // Tentativa 2: PLANO DE EMERGÊNCIA!
        // Se o navegador bloquear o fetch por estar a abrir sem servidor, isto salva a pátria:
        console.warn("Fetch bloqueado pelo navegador. A ativar o Plano de Emergência para a Vitrine!");
        ...
        const jsonTexto = `{
            "catalogo": [
                ...
                {"id": 9, "titulo": "Edital Concurso 2026", "autor": "Documento Oficial", "ano": 2026, "genero": "Documentos", "capa": "IMG/documentos/edital.jpeg" },
                ...
                {"id": 14, "titulo": "ENEM- Ciências da Natureza", "autor": "Apostilas Opcao", "ano": 2025, "genero": "Ensino", "capa": "IMG/ensino/apostilaEnem.png" },
        ...
        const dadosEmergencia = JSON.parse(jsonTexto);
        CATALOGO_LIVROS = dadosEmergencia.catalogo;
        BIBLIOTECAS_USUARIOS = dadosEmergencia.bibliotecas;
    }
}